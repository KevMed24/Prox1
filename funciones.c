...
void leerClientes(char clientes[5][2][40], const char *filename) {
    ...
    for (int i = 0; i < 5; i++) {
        if (fscanf(file, "%39s %39s", clientes[i][0], clientes[i][1]) != 2) {
            strcpy(clientes[i][0], "");
            strcpy(clientes[i][1], "");
        }
    }
...
void leerReservas(int reservas[10][4], const char *filename) {
    FILE *file = fopen(filename, "r");
    if (file == NULL) {
        printf("No se puede abrir el archivo de reservas\n");
        return;
    }
    for (int i = 0; i < 10; i++) {
...
void guardarClientes(char clientes[5][2][40], const char *filename) {
    FILE *file = fopen(filename, "w");
    if (file == NULL) {
        printf("No se puede abrir el archivo de clientes para escribir\n");
        return;
    }
    for (int i = 0; i < 5; i++) {
...
void guardarReservas(int reservas[10][4], const char *filename) {
    FILE *file = fopen(filename, "w");
    if (file == NULL) {
        printf("No se puede abrir el archivo de reservas para escribir\n");
        return;
    }
    for (int i = 0; i < 10; i++) {
